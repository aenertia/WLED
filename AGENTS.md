...
### Output Gate for AI-Assisted Contributions

Before emitting any PR description, issue body, commit message, review reply, or
discussion comment, apply the **oss-contributor-epistemics** output gate. This is
mandatory, not advisory.

The gate is grounded in the psychological mechanisms that cause AI-assisted OSS
contributions to fail even when the underlying work is correct: the Heuristic of
Sufficient Explanation (Vonasch et al. 2024), correspondence bias (Gilbert &
Malone 1995), SRT anchoring (Moscovici; Wagner & Kronberger 2019), displaced
aggression (Marcus-Newhall et al. 2000; Pedersen et al. 2008), face-threatening
acts (Brown & Levinson 1987), burden-of-proof inversion, and emotional contagion
in OSS threads (Ortu et al. 2020).

The short version of what the gate enforces:

- **Uncertainty is explicit and evidence-bearing.** "I believe the root cause is
  X, but I haven't ruled out Y" is required when making unverified diagnostic
  claims. "I'm not sure" without a mental model attached is not sufficient — bare
  uncertainty reads as absence of understanding and triggers the Heuristic of
  Sufficient Explanation.

- **No hypothesis generated to fill silence.** When a correction arrives without
  explanation, the correct response is a routing question directed at the
  maintainer's domain knowledge — not a new hypothesis produced under social
  pressure to justify continued engagement. A wrong hypothesis stated with
  confidence anchors the thread as a red herring.

- **Symptom data is separated from hypothesis.** Observed behaviour under stated
  conditions is stated separately from the proposed mechanism. The maintainer has
  the domain knowledge to generate hypotheses; the contributor has the symptom
  data. Route accordingly.

- **Disclosure is one sentence, factual, non-apologetic.** "Assisted-by: [tool]"
  in the commit message or PR description. A paragraph of hedging pre-anchors the
  reviewer to the "AI slop" category before they read the content.

- **Existing comments are preserved.** AI tools routinely delete comments they
  consider redundant. They are not redundant. Restore any deleted before
  submitting. Diff against the original.

- **Replies to corrections are ≤150 words.** Length in response to brevity reads
  as confidence-performance, not correction-processing, and increases the
  probability of the next response being negative (emotional contagion).

WLED-specific additions to the gate:

- Use `// AI: below section was generated by an AI` / `// AI: end` markers for
  AI-generated code blocks (see Attribution section above). This is the
  project-specific form of the gate's disclosure requirement.
- The WLED bug template explicitly asks reporters not to submit "long error
  analyses created by an AI agent." The gate's §4 issue template (hypothesis
  section optional, symptom data mandatory) is the correct form for WLED issues.
- When softhack007, Aircoookie, or other maintainers apply the `AI` label to a
  PR, do not respond to the label directly. Continue engaging with the technical
  content. The label is an anchoring event (M3); the only way to shift the frame
  is through the content, not through meta-discussion about the label.
