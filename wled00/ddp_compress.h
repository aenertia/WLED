...
struct RLEDecoder {
  ...
  size_t maxDecoded; // output ceiling (defense-in-depth against malformed input)
  size_t decoded;    // bytes decoded so far

  void init(const uint8_t *data, size_t len, size_t maxOut = SIZE_MAX) {
    src = data; srcLen = len; pos = 0; remaining = 0;
    maxDecoded = maxOut; decoded = 0;
  }

  // Decode next byte. Returns false when input exhausted or output ceiling reached.
  bool next(uint8_t *out) {
    if (decoded >= maxDecoded) return false;
    if (remaining > 0) {
      remaining--;
      if (isRun) {
        *out = value;
