...
// Returns true if any segment overlapping [busStart, busStart+busLen) is active
// (on && !freeze) or held by DDP realtime. Used by BusManager::show() to skip
...
// AXP192 power rail init (M5StickC). Called from WLED::setup() before beginStrip().
// Register map (I2C 0x34, Wire1 SDA=21 SCL=22):
...
// Without this, the unpowered mic CLK pulls GPIO0 low (boot strapping pin  ->
// download mode). See m5stack/M5StickC-Plus#1.
...
bool initAXP192() {
  ...
  // Probe reg 0x12  -- bail if AXP192 not present rather than hanging on writes.
  bool probeOk = false;
  ...
  if (!axp192WriteChecked(0x28, 0xCC)) return false;  // LDO2+LDO3 = 3.0V

  bool rdOk = false;
  uint8_t reg12 = axp192ReadChecked(0x12, &rdOk);
  if (!rdOk) return false;
  if (!axp192WriteChecked(0x12, reg12 | 0x0C)) return false;  // enable LDO2+LDO3

  // GPIO0  -> LDOio0 mode: mic CLK on GPIO0 (boot strapping pin) must be driven,
  // not floating, or it pulls low and forces download mode on reset.
  if (!axp192WriteChecked(0x90, 0x02)) return false;
  if (!axp192WriteChecked(0x91, 0xA0)) return false;  // LDOio0 = 2.8V (SPM1423)
...
// Raw I2C wrappers (no error checking  -- callers handle failure via state).
void BusSPIMatrix::axpWrite(uint8_t reg, uint8_t val) {
  Wire1.beginTransmission(0x34);
  Wire1.write(reg);
  Wire1.write(val);
...
uint8_t BusSPIMatrix::axpRead(uint8_t reg) {
  Wire1.beginTransmission(0x34);
  Wire1.write(reg);
...
, _panelWidth(SPI_MATRIX_W)
, _panelHeight(SPI_MATRIX_H)
...
, _activeBuf(0)
{
...
#ifdef WLED_SPI_MATRIX_AXP192
  if (!initAXP192()) {
    DEBUG_PRINTLN(F("BusSPIMatrix: AXP192 init failed  -- marking bus invalid"));
    _valid = false;
    return;
  }
#endif
  ...
  if (!_spiDisplay) {
    ...
    _spiDisplay->init();
    _spiDisplay->setRotation(0);
    _spiDisplay->setSwapBytes(false);
    _spiDisplay->initDMA();
    _spiDisplay->fillScreen(TFT_BLACK);
  }

  // width()/height() return post-rotation size  -- correct for any rotation.
  _scaleX = _spiDisplay->width()  / _panelWidth;
  _scaleY = _spiDisplay->height() / _panelHeight;
  if (_scaleX < 1) _scaleX = 1;
  if (_scaleY < 1) _scaleY = 1;

  // DMA strip height: largest that fits within SPI DMA limit (32767 px) and
  // available DMA heap budget. Fewer strips = fewer pushPixelsDMA calls.
  ...
  const size_t bytesPerPhysStrip = bytesPerRow * _scaleY;  // scaleY physical rows per virtual row
  ...
  // Buffers allocated lazily on first active show()  -- saves ~28KB at boot.
  // _skipShow=true: skip the beginStrip() show() before NVS state loads.
  _valid = (_spiDisplay != nullptr);
  DEBUGBUS_PRINTF_P(PSTR("SPI Matrix: %dx%d (%d pix), scale %dx%d, dmaRows=%d (%u B/strip), heap: %u free, %u budget, valid=%d (buffers deferred)\n"),
...
void BusSPIMatrix::show() {
  ...
  for (uint16_t s = 0; s < numStrips; s++) {
    ...
    if (dmaStarted) _spiDisplay->dmaWait();
    dmaStarted = true;
    ...
    _spiDisplay->pushImageDMA(0, stripY * _scaleY, physW, physRows, buf);
    _activeBuf ^= 1;
  }
...
size_t BusSPIMatrix::getPins(uint8_t* pinArray) const {
  if (pinArray) {
    pinArray[0] = TFT_MOSI;
    pinArray[1] = TFT_SCLK;
    pinArray[2] = TFT_CS;
...
std::vector<LEDType> BusSPIMatrix::getLEDTypes() {
  return {
    {TYPE_SPI_MATRIX, "", PSTR("SPI Matrix")},
  };
}

// Compute [_activeRowMin, _activeRowMax)  -- the virtual row range with live segment
// coverage. Called every frame; only DMA strips in this range get pushed.
void BusSPIMatrix::recalcActiveRowRange() {
  _prevActiveRowMax = _activeRowMax;
  ...
  for (unsigned i = 0; i < strip.getSegmentsNum(); i++) {
    ...
    // 2D: convert column coords to flat pixel range (same as busHasActiveSegment).
    uint16_t segPixStart, segPixEnd;
    if (is2D) {
      segPixStart = (uint16_t)seg.startY * mw + seg.start;
      segPixEnd   = (uint16_t)seg.stopY * mw;
    ...
    uint16_t rowEnd   = (localEnd + _panelWidth - 1) / _panelWidth;  // ceil div
...
bool BusSPIMatrix::allocateBuffers() {
  ...
  if (!_dmaBuf[0] || !_dmaBuf[1]) {
    // OOM  -- halve dmaRows and retry once
...
#endif
#ifdef WLED_ENABLE_SPI_MATRIX
  } else if (Bus::isSPIMatrix(bc.type)) {
    busses.push_back(make_unique<BusSPIMatrix>(bc));
#endif
...
String BusManager::getLEDTypesJSONString() {
  ...
  #endif
  #ifdef WLED_ENABLE_SPI_MATRIX
  json += LEDTypesToJson(BusSPIMatrix::getLEDTypes());
  #endif
